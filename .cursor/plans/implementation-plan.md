...
Current state: Laravel 12 skeleton in `app/`, Docker (`app` + `nginx` on port 8081), API routes not registered, `nwidart/laravel-modules` not installed.
...
- Register API routing: prefix `/api`
- Generate modules: `AppIcon`, `AppleStore`, `GooglePlay`
- Create `app/Shared/` (DTO, Exceptions)

---

### 1. Adapter contract tests → contracts & adapters

**Tests (red):**
- `AppleIconProvider` resolves to `AppleStoreIconAdapter`
- `GooglePlayIconProvider` resolves to `GooglePlayIconAdapter`

**Implementation (green):**
- Contracts in `AppIcon/Contracts/`
- Adapters in `AppleStore`, `GooglePlay`
- DI bindings in each store module ServiceProvider
...
**Phase 1:**
1. `add modules scaffold and api v1 routing`
2. `add adapter contract tests and provider bindings`
